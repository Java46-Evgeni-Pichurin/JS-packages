...
export default class College {
    ...
    getHoursStatistics(lengthInterval){
        return _.map(_.countBy(this.getAllCourses(), (course) => Math.floor(course.hours / lengthInterval)),
                (val, key) => ({minInterval: key * 10, maxInterval: key * 10 + lengthInterval - 1, amount: val}));
    }
    getCostStatistics(lengthInterval){
        return _.map(_.countBy(this.getAllCourses(), (course) => Math.floor(course.cost / lengthInterval)),
                (val, key) => ({minInterval: key * 2000, maxInterval: key * 2000 + lengthInterval - 1, amount: val}));
    }
}